...
function buildSystemPrompt(data: InstallmentData[]): string {
...
app.get("/api/gemini/status", (_request, response) => {
  response.json({ configured: !!process.env.GEMINI_API_KEY });
});
...
app.post("/api/gemini/analyze", async (request, response) => {
  ...
  try {
    ...
    const parsed = JSON.parse(result.text || "[]") as InstallmentData[];
...
app.post("/api/gemini/chat", async (request, response) => {
  ...
  try {
    ...
    response.json({ data: { text: result.text || "" } });
  } catch (error) {
    sendGeminiError(response, error);
  }
});
